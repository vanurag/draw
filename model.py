...
#     self.mus, self.logsigmas, self.sigmas = [0] * self.T, [0] * self.T, [0] * self.T  # gaussian params generated by SampleQ. We will need these for computing loss.
  ...
  def write_decision(self, h_dec):
    ...
    with tf.variable_scope("write_decision", reuse=self.DO_SHARE):
      sw_log_odds = self.linear(h_dec, 1)
  ...
  def draw_loop_body(self, t, stop_sum, stop_times, cs, scs, read_bb, write_bb, mus, logsigmas, sigmas,
...
#     cs = cs.write(t, c_prev + write_output[0])
    ...
    return [tf.add(t, 1), stop_sum, stop_times, cs, scs, read_bb, write_bb, mus, logsigmas, sigmas,
            should_write_log_odds, should_write_pre_sigmoid, h_dec_prev, enc_state, dec_state]
...
#       tf.reduce_any(tf.less(stop_sum, self.stop_writing_threshold)))
  ...
  def build_model(self):
    ...
    with tf.variable_scope('draw_model'):  # , reuse=self.DO_SHARE):
      ...
      t, stop_sum, self.stop_times, self.cs, self.scs, self.read_bb, self.write_bb, self.mus, self.logsigmas, self.sigmas, \
...
#         x_recons = tf.nn.tanh(self.cs.read(self.T - 1))
        ...
        tf.summary.scalar('Write Loss', self.Lwrite, collections=[self.summary_collection])
        ...
        t, self.write_bb, total_movement = \
          tf.while_loop(lambda t, a, b: tf.less(t, self.T - 1), _movement_loss_loop_body,
                        [t, self.write_bb, total_movement],
                        parallel_iterations=1)
        self.Lmove = 10.0 * scaling_factor * tf.reduce_mean(total_movement)
...
#         Lssim = tf.reduce_mean((1 - tf.image.ssim(self.input_, x_recons, max_val=1.0)) / 2)
        ...
        # Total loss
        self.loss = self.Lx + self.Lz + self.Lwrite
